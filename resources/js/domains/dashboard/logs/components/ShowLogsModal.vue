<template>
    <div class="mb-6">
        <div class="flex justify-center mb-4">
            <h1 class="text-base font-bold leading-6 text-gray-900 text-xl">{{
                classStore.getters.byId(props.class_id).value.name }}</h1>
        </div>
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
            ...
            <div class="overflow-hidden transition hover:scale-105 rounded-lg bg-gray-100 px-4 py-5 shadow sm:p-6">
                ...
                <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ classInfo.students.length }}</dd>
            </div>

            <div class="overflow-hidden transition hover:scale-105 rounded-lg bg-gray-100 px-4 py-5 shadow sm:p-6">
                <dt class="truncate text-sm font-medium text-gray-500">Totale aanwezigheid</dt>
                <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{
                    allClockedInByClocks(clocks, classInfo).length }} /
                    {{ classInfo.students.length }}</dd>
            </div>

            <div class="overflow-hidden transition hover:scale-105 rounded-lg bg-gray-100 px-4 py-5 shadow sm:p-6">
                <dt class="truncate text-sm font-medium text-gray-500">Ziek</dt>
                <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">
                    {{ clocks.filter(clock => clock.status.toString() === 'ziek' && classInfo.students.find((student) =>
                        student.student_class_id == classInfo.id)).length }}
                </dd>
            </div>
        </dl>
    </div>
    ...
    <div class="flex mb-6">
        <section class="w-full">
            <details open
                class="w-full bg-gray-100 p-4 rounded-xl shadow-md group mx-auto overflow-hidden max-h-[56px] open:!max-h-[400px] transition-[max-height] duration-500 overflow-hidden">
                <summary
                ...
                <div class="">
                    <table class="table-auto border-collapse w-full">
                        <thead>
                            <tr class="rounded-lg text-sm bg-grey-200 font-medium text-gray-700 text-left"
                        ...
                        <tbody class="text-gray-700 bg-white">
                            <tr v-for="student in students" :key="student.id"
                                ...
                                <td class="px-4 py-4">{{ clockPresentation(clocks.find((clock) => clock.student_id ==
                                    student.id)) }}
                                </td>
                            </tr>
                        </tbody>
...
const students = computed(() => studentStore.getters.all.value.filter((student) => student.student_class_id == props.class_id));
