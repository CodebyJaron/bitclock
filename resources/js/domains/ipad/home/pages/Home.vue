        ...
        <template #default>
            <div class="flex items-center justify-center min-h-screen bg-gray-100">
                <div class="px-16 py-12 mt-4 text-left bg-white shadow-lg">
                    ...
                    <p v-if="selectedClass" class="italic text-xs">Geselecteerde klas: {{
                        classStore.getters.byId(selectedClass).value.name }}</p>
                </div>
            </div>
        </template>
...
const submitCheckIn = async () => {
    if (clockStore.getters.findTodayByStudent(selectedStudent.value)) {
        ...
        return infoToast("Je bent al ingeklokt.");
    }

    formModal(choosedExcercise.value, defineAsyncComponent(() => import('../components/ExcerciseForm.vue')),
        async (excercise: string) => {
            ...
            if (clock && is_late(clock)) {
                await studentStore.actions.addMinutesToStudent(studentStore.getters.byId(selectedStudent.value).value, getToLateTime(clock));
                showModal(defineAsyncComponent(() => import('../components/LateModal.vue')), { minuten: getToLateTime(clock) })
            }
